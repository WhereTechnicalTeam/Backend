...
class ImageUpload(APIView):
	...
	def get(self, request, pk):
		...
		if usr:
			if ImageProfile.objects.filter(user_profile_id=usr[0].id):				
				queryset = ImageProfile.objects.get(user_profile_id=usr[0].id)
				serializer = ImageSerializer(queryset)
				return Response(serializer.data, status=status.HTTP_200_OK)
			return Response({"status":status.HTTP_400_BAD_REQUEST, "msg":"image not found"})
		else:
			return Response({"status":status.HTTP_400_BAD_REQUEST, "msg":"image not found"})
	...
	def put(self, request, format=None):
		#print(request.data.get('user_profile'))

		try:
			if UserProfile.objects.filter(user_id=request.data.get('user_profile')).exists: 
				userp = UserProfile.objects.filter(user_id=request.data.get('user_profile'))[0]

				#print(userp)
				data, updated = ImageProfile.objects.filter(user_profile_id=userp.id).update_or_create(
				...
				print(data.image)
				return Response({"status":status.HTTP_200_OK, "msg":"Image uploaded", "image": '{}'.format(data.image)})
